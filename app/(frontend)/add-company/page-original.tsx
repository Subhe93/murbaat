    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (countriesRes.ok) {
          ...
          console.error('فشل في تحميل البلدان:', countriesRes.status);
          const errorData = await countriesRes.json().catch(() => ({}));
          console.error('خطأ البلدان:', errorData);
        }
        ...
        if (categoriesRes.ok) {
          ...
          console.error('فشل في تحميل الفئات:', categoriesRes.status);
          const errorData = await categoriesRes.json().catch(() => ({}));
          console.error('خطأ الفئات:', errorData);
        }
      } catch (error) {
      ...
      if (formData.countryId) {
        try {
          ...
          if (response.ok) {
            ...
            console.error('فشل في تحميل المدن:', response.status);
            const errorData = await response.json().catch(() => ({}));
            console.error('خطأ المدن:', errorData);
            setCities([]);
          }
        } catch (error) {
