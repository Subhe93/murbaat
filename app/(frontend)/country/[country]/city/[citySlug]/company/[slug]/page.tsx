...
export async function generateStaticParams() {
  try {
    ...
    return result.data.map((company) => ({
      country: encodeURIComponent(company.country.code),
      citySlug: encodeURIComponent(company.city.slug),
      slug: encodeURIComponent(company.slug),
    }));
  } catch (error) {
...
export async function generateMetadata({ 
  ...
  params: { country: string; citySlug: string; slug: string } 
}): Promise<Metadata> {
  try {
...
export default async function CompanyPage({ 
  ...
  try {
    ...
    const decodedSlug = decodeURIComponent(params.slug);
    const decodedCitySlug = decodeURIComponent(params.citySlug);
    const decodedCountry = decodeURIComponent(params.country);
    
    const company = await getCompanyBySlug(decodedSlug);
    
    if (!company || 
        company.country.code !== decodedCountry.toLowerCase() || 
        company.city.slug !== decodedCitySlug) {
      notFound();
    }
