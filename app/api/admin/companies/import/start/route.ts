...
async function processImportInBackground(importId: string) {
  ...
  try {
    for (let i = session.currentIndex; i < session.data.length; i++) {
      ...
      try {
        // معالجة الصف الحالي
        const result = await importService.processCompanyRow(row, session.settings, i + 2) // +2 للرأس والفهرسة
    ...
    const errorMessage = error instanceof Error ? error.message : 'خطأ في النظام'
    importSessionManager.updateSession(importId, { 
      status: 'failed',
      errors: [...session.errors, {
        row: 0,
