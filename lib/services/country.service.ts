...
// جلب بيانات البلد
export const getCountryByCode = cache(async (countryCode: string): Promise<CountryPageData | null> => {
  try {
...
// جلب مدن البلد
export const getCountryCities = cache(async (countryCode: string): Promise<CountryCity[]> => {
  try {
...
export const getCountryCompanies = cache(async (
  countryCode: string, 
  ...
  try {
    ...
    if (filters.searchQuery) {
      where.OR = [
        ...
        { shortDescription: { contains: filters.searchQuery, mode: 'insensitive' } },
...
export const getCountryFilterOptions = cache(async (countryCode: string): Promise<FilterOptions> => {
  try {
